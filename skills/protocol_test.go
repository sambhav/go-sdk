...
func TestLimitsRoundTrip(t *testing.T) {
	for _, kind := range []string{"count", "bytes"} {
		t.Run(kind, func(t *testing.T) {
			...
			if kind == "count" {
				limits.MaxResourcesPerSkill++
				entries, _ := skill.Resources.List()
				for i := 1; i < limits.MaxResourcesPerSkill; i++ {
				...
				limits.MaxTotalSize++
				entries, _ := skill.Resources.List()
				entries[0].Size = limits.MaxTotalSize
			}
			server := testServer()
			...
			if _, err := client.List(t.Context(), nil); err == nil {
				t.Fatal("default client accepted oversized list")
			}
			if _, err := client.Get(t.Context(), &GetSkillParams{URI: skill.URI}); err == nil {
				t.Fatal("default client accepted oversized get")
			}
			...
			// A higher budget never bypasses structural checks.
			malformed := *skill
			malformed.Frontmatter = Frontmatter{"name": "BAD", "description": "Demo"}
			if err := ValidateSkillWithLimits(&malformed, limits); err == nil {
				t.Fatal("accepted malformed skill")
			}
			// The server also enforces its own defaults.
			if err := AddHandlers(server, fixedHandlers(skill), nil); err != nil {
				t.Fatal(err)
			}
			if _, err := client.Get(t.Context(), &GetSkillParams{URI: skill.URI}); err == nil {
				t.Fatal("default server served oversized skill")
			}
		})
	}
	if err := ValidateSkillWithLimits(testSkill(), Limits{MaxTotalSize: -1}); err == nil {
		t.Fatal("negative limit accepted")
	}
	if err := AddHandlers(testServer(), fixedHandlers(testSkill()), &ServerOptions{Limits: Limits{MaxResourcesPerSkill: -1}}); err == nil {
		t.Fatal("negative server limit accepted")
	}
	entries, _ := testSkill().Resources.List()
	skill := testSkill()
	entries[0].Size = DefaultMaxTotalSize
	skill.Resources = StaticResources(entries...)
	if err := ValidateSkillWithLimits(skill, Limits{MaxResourcesPerSkill: 1}); err != nil {
		t.Fatal(err)
	}
	entries[0].Size++
	if err := ValidateSkillWithLimits(skill, Limits{MaxResourcesPerSkill: 1}); err == nil {
		t.Fatal("zero byte limit disabled the default")
	}
}
